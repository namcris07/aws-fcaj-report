...
Để xác minh tính đúng đắn và hiệu quả của hệ thống `devsecops-factory`, nhóm đã thiết kế ứng dụng **[tetris-app](https://github.com/lamelihuynh/tetris-app.git)** (ReactJS + Nginx multi-stage build) làm mục tiêu kiểm thử có chủ ý. Ứng dụng được cài đặt sẵn các lỗ hổng ở nhiều tầng khác nhau, bao gồm mã nguồn, thư viện phụ thuộc, cấu hình container Dockerfile và cấu hình hạ tầng AWS (Terraform, ECS Task Definitions).

Mục này phân tích chi tiết kết quả kiểm thử qua **6 Security Gates** trong pipeline CI/CD Jenkins 11 stage, quá trình tập trung báo cáo lên **Amazon S3**, xử lý sự cố tự động qua **AWS Lambda**, và triển khai ứng dụng an toàn lên cụm máy chủ **Amazon ECS Fargate** kết hợp với **AWS CloudWatch Container Insights**.

...
- **Purpose:**
  ...
  - To share cloud-native architectural solutions on AWS and practical insights from award-winning Hackathon teams of the Agentic AI Build Week (co-organized with GenAI Fund).
...
- **Speakers & competing teams:**
  ...
  - **Signal Scout (2nd Place – AWS Track):** Hoang Hieu, Trieu Quoc Hao, Minh Quan (Willer), Nguyen Cong Minh, Nguyen Van Duy Khiem, Le Tan Luc.
  - **Plan V Team (Solution Architect Professional Native App):** Huynh Hoang Long, Tran Dai Vi, Pham Tien Thuan Phat, Nguyen An, Le Minh Nghia.
  ...
  - **Six Pillars Team (Adaptive Workflow Engine):** Viet, Nguyen Van Linh, Nguyen, Minh Nhat, Phuoc Huyen.

## Key Content

### 1. Opening Keynote: Reframing the Mental Model for the AI Agent Era (Joseph Marazota)
...
### 2. AI-Powered Conversational Ordering – KFC Agent (One Team – 1st Place)
...
### 3. Multi-Agent Competitor Intelligence Platform – Signal Scout (Signal Scout – 2nd Place)
...
- **Multi-Agent Architecture (A2A – Agent to Agent):**
  ...
  - **Analysis Subagent & Validation Loop:** Uses **Langfuse** to score data quality. If the score is insufficient, the system automatically retries retrieval (up to 2 times to preserve token budget); only unresolved cases escalate to human review.
...
### 4. SA Professional AI Native App (Plan V Team)
...
- **AI Native App Workflow:**
  ...
  - $\rightarrow$ AI analyzes enterprise policies and constraints
  - $\rightarrow$ Automatically generates architecture diagrams on **Draw.io** (using official AWS icons)
  - $\rightarrow$ Automatically computes a **Cost Estimation** breakdown (for region ap-southeast-1)
  - $\rightarrow$ Auto-generates **IaC code (Terraform/CloudFormation)**
  - $\rightarrow$ Automatically deploys the infrastructure to AWS.
...
### 5. S.H.E.P.H.E.R.D. – Smart Human-flow Evaluation, Prediction, Hazard Detection, Response, and Dispatch (3KA Team)
...
- **Technical Architecture:**
  ...
  - Integrates **Bedrock Agent** (Autonomous Monitor & Operator Copilot) to synthesize analysis and generate automated staff coordination recommendations.

### 6. Anti-Money Laundering Detection Framework – Adaptive Workflow Engine (Six Pillars)
...
- **3-Layer Processing Architecture:**
  - **Layer 1 – Fast Detection:** **Kinesis Data Stream** ingests data $\rightarrow$ **Lambda Feature Engineering** $\rightarrow$ **XGBoost** classifier rapidly flags anomalous transactions (low cost, filters out 90–95% of non-suspicious activity).
...
| **Competitor Intelligence** | Manual research teams, slow turnaround, high cost ($94+/month). | Multi-Agent A2A (Supervisor $\rightarrow$ Crawler $\rightarrow$ Analysis + Validation Loop + Langfuse); reduced to $35/month with Native AWS Tools. |
| **Architecture Design & Estimation** | SA spends hours hand-drawing, calculating costs in spreadsheets. | AI Native App (Plan V Team): Natural language $\rightarrow$ Draw.io diagram $\rightarrow$ Cost Estimation $\rightarrow$ Terraform $\rightarrow$ auto-deploy; significant time savings. |
| **Crowd Monitoring** | Passive CCTV review after congestion occurs. | Kinesis Video + YOLO + ByteTrack + Bedrock Agent (S.H.E.P.H.E.R.D.): real-time analysis with proactive staff recommendations. |
| **AML Detection** | 90–95% False Positive rate, 3 hrs/case, $20–$25/manual review. | 3-Layer (XGBoost $\rightarrow$ Step Functions/RAG $\rightarrow$ Double LLM + Guardrails); only genuine high-risk cases escalate. |
...
- **Interactive Workshop x Hackathon Showcase:** Delivered an engaging, open, and practical hands-on atmosphere.
